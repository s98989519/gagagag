...
const Game = {
    ...
    showBlacksmithConfirm(idx) {
        ...
        const rate = getBlacksmithRate(enhance).rate;
        if (Player.gold < cost) {
            alert("金幣不足！");
            return;
        }
        ...
        const valIncrease = nextVal - currentVal;
        ...
        const msg = `🔨 強化確認\n\n` +
            `裝備名稱: ${item.name}${enhance > 0 ? ` +${enhance}` : ''}\n` +
            `消耗成本: ${cost} G + 1個 ${baseName}\n` +
            `成功機率: ${rate}%\n` +
            `━━━━━━━━━━━━━━━━━━━━\n` +
            `📊 ${statType}變化:\n` +
            `   當前: ${currentVal}\n` +
            `   成功後: ${nextVal} (+${valIncrease}) ✨\n` +
            `   失敗: ${currentVal} (裝備不變，素材消失)\n` +
            `━━━━━━━━━━━━━━━━━━━━\n` +
            `確定要進行強化嗎？`;

        if (confirm(msg)) {
            this.attemptEnhance(idx);
        }
    },
    ...
    attemptEnhance(idx) {
        ...
        const baseName = item.name.replace(/\s*\+\d+$/, '');
        const duplicateIdx = Player.inventory.equipment.findIndex((i, index) => {
        ...
        Player.inventory.equipment.splice(duplicateIdx, 1);
        ...
        if (success) {
            ...
            const baseName = item.name.replace(/\s*\+\d+$/, '');
            item.name = `${baseName} +${item.enhance}`;
            this.showFloatingText("強化成功!", "#69f0ae");
            this.triggerAnim('event-icon', 'anim-spawn');
            const newVal = item.val + Math.floor(item.val * item.enhance * 0.1);
            item.val = newVal;
            this.renderEvent("✨ 強化成功！", `${item.icon} ${item.name}`, `恭喜！裝備變得更強了！<br><br>當前${item.type === 'weapon' ? '攻擊力' : item.type === 'armor' ? '生命值' : '耐久'}: <span style="color:#69f0ae">${newVal}</span>`, "🎉");
        } else {
            this.showFloatingText("強化失敗...", "#ff5252");
            this.triggerAnim('game-container', 'anim-screen-shake');
            this.renderEvent("💔 強化失敗", "工匠嘆了口氣...", `${item.name} 強化失敗，但裝備未受損。<br><span style="color:#888">素材和金幣已消耗。</span>`, "😔");
        }
        this.updateUI();
        setTimeout(() => {
            if (GameState.blacksmithAttempts < 2) {
