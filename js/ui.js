...
const UISystem = {
    ...
    showStatDetails(type) {
        if (!window.Game.getStatBreakdown) return;
        const details = window.Game.getStatBreakdown(type);
        if (!details) return;

        const modal = document.getElementById('stat-detail-modal');
        const titleEl = document.getElementById('stat-detail-title');
        const container = document.getElementById('stat-detail-container');

        if (!modal || !container) return;

        // Set Title
        if (type === 'evasion') titleEl.innerText = "💨 閃避率詳情";
        if (type === 'crit') titleEl.innerText = "🎯 暴擊率詳情";
        if (type === 'atk') titleEl.innerText = "⚔️ 攻擊力詳情";
        if (type === 'def') titleEl.innerText = "🛡️ 防禦力詳情";

        container.innerHTML = "";

        // Helper to create stat row
        const createRow = (label, value, color, icon) => {
            if (!value && value !== 0) return; // Skip if undefined/null
            if (value === 0 && label !== '總計') return; // Optional: Hide 0 values? Or keep for clarity? Let's keep non-zero only for cleaner look, except Total.
            ...
            // Determine Unit
            let unit = "%";
            if (type === 'atk' || type === 'def') unit = "";

            let valStr = `${value > 0 ? '+' : ''}${value}${unit}`;
            // If it's Total Base/Total, maybe don't show +?
            if (label === '基礎數值' || label === '總計') {
                valStr = `${value}${unit}`;
            }

            div.innerHTML = `
                <span style="color:#ddd; font-size:1em;">${icon} ${label}</span>
                <span style="color:white; font-weight:bold; font-size:1.1em;">${valStr}</span>
            `;
            return div;
        };

        // 1. Base
        if (details.base > 0) container.appendChild(createRow("基礎數值", details.base, '#9e9e9e', '🏳️')); // Gray

        // 2. Equip
        if (details.equip > 0) container.appendChild(createRow("裝備加成", details.equip, '#2196f3', '🛡️')); // Blue

        // 3. Buff
        if (details.buff > 0) container.appendChild(createRow("狀態加成", details.buff, '#ff9800', '✨')); // Orange

        // 4. Other
        if (details.other > 0) container.appendChild(createRow("其他加成", details.other, '#9c27b0', '🔮')); // Purple
        ...
        // 5. Total
        const totalDiv = createRow("總計", details.total, '#69f0ae', '📊'); // Green
        container.appendChild(totalDiv);

        // [NEW] 6. 防禦減傷百分比 (僅防禦顯示)
        if (type === 'def' && details.total !== null && details.total !== undefined) {
            const damageMultiplier = 100 / (100 + details.total);
            const damageReduction = (1 - damageMultiplier) * 100;
            const drPercent = Math.round(damageReduction);

            const drDiv = document.createElement('div');
            drDiv.style.background = '#1e3a5f';
            drDiv.style.padding = '12px';
            drDiv.style.borderRadius = '5px';
            drDiv.style.marginTop = '10px';
            drDiv.style.textAlign = 'center';
            drDiv.style.border = '2px solid #4caf50';

            drDiv.innerHTML = `
                <div style="color: #4caf50; font-size: 0.9em; margin-bottom: 5px;">💪 實際減傷效果</div>
                <div style="color: #ffffff; font-size: 1.3em; font-weight: bold;">${drPercent}%</div>
                <div style="color: #aaa; font-size: 0.85em; margin-top: 5px;">受到的傷害會減少 ${drPercent}%</div>
            `;
            container.appendChild(drDiv);
        }

        // 7. Cap (Special styling)
        if (details.cap !== null) {
            const capDiv = document.createElement('div');
            capDiv.style.background = '#222';
            capDiv.style.padding = '8px';
            capDiv.style.borderRadius = '5px';
            capDiv.style.marginTop = '5px';
            capDiv.style.textAlign = 'center';
            capDiv.style.border = '1px dashed #ff5252';

            let capText = `⚠️ 系統上限: ${details.cap}%`;
            if (details.total > details.cap) {
                capText += ` (已溢出 ${details.total - details.cap}%)`;
            }

            capDiv.innerHTML = `<span style="color:#ff5252; font-size:0.9em;">${capText}</span>`;
    ...
    getIntentHtml(enemy) {
        ...
        switch (enemy.nextAction.type) {
            ...
            case 'flee':
                ...
                color = "gray";
                break;
            case 'prepare': // [New] 史萊姆之王 (Slime King) 準備動作
                icon = "⚠️";
                text = "準備中";
                color = "orange";
                break;
    ...
    setCombatButtons(mainText, mainAction, subText, subAction, defAction) {
        ...
        if (bUlt) {
            ...
            // [FIX] 動態計算 SP 需求 (所有職業 Lv5: 7, Lv6: 6)
            let requiredSp = 8; // 預設
            ...
            if (window.Player.sp >= requiredSp) {
                ...
                bUlt.disabled = false;
                bUlt.innerHTML = "🔥 終結技 (已就緒)";
            } else {
                bUlt.classList.remove('glow-gold');
                bUlt.disabled = true;
                bUlt.innerHTML = `🔥 終結技 (${window.Player.sp}/${requiredSp})`;
            }
        }
    ...
    updateStatsUI() {
        ...
        const atkVal = window.Game.getAtk();
        const atkEl = document.getElementById('atk-val');
        if (atkEl) {
            atkEl.innerText = atkVal;
            atkEl.style.cursor = 'help';
            atkEl.style.textDecoration = 'underline dotted';
            atkEl.onclick = () => this.showStatDetails('atk');
        }

        // --- 新增：計算防禦減免率 ---
        const totalDef = window.Game.getDef();
        // 正確的減傷公式：reduction = 100 / (100 + Def)，所以減傷% = (1 - reduction) * 100
        const damageMultiplier = 100 / (100 + totalDef); // 傷害倍率
        const damageReduction = (1 - damageMultiplier) * 100; // 減傷百分比
        const drDisplay = Math.round(damageReduction); // 四捨五入

        // --- 修正防禦力面板顯示，讓它更有意義 ---
        const defEl = document.getElementById('def-val');
        if (defEl) {
            defEl.innerText = `${totalDef} (${drDisplay}%減傷)`;
            defEl.style.cursor = 'help';
            defEl.style.textDecoration = 'underline dotted';
            defEl.onclick = () => this.showStatDetails('def');
        }

        document.getElementById('crit-val').innerText = window.Game.getCrit() + '%';

        // [New] Make Stats Clickable
        const critEl = document.getElementById('crit-val');
        if (critEl) {
            critEl.style.cursor = 'help';
            critEl.style.textDecoration = 'underline dotted';
            critEl.onclick = () => this.showStatDetails('crit');
        }

        // [New] 顯示閃避率
        const evasionVal = window.Game.getEvasion();
        const evaEl = document.getElementById('evasion-val');
        if (evaEl) {
            let displayVal = evasionVal;
            // Visual indicator for cap
            if (evasionVal >= 85) displayVal = "85 (MAX)";

            evaEl.innerText = displayVal + '%';
            evaEl.style.cursor = 'help';
            evaEl.style.textDecoration = 'underline dotted';
            evaEl.onclick = () => this.showStatDetails('evasion');
        }
        ...
        if (window.GameState.harpyThreat) {
            if (hasBuff) buffHtml += " | ";
            buffHtml += `<span class="damage-text" style="color:#ff5722">🦅 來自空中的威脅</span>`;
            buffAlertText += `【來自空中的威脅】\n受到哈比的精準盯防！\n逃跑率降低 40%，且逃跑失敗時會被偷竊物品！\n\n`;
            hasBuff = true;
        }

        if (hasBuff) {
            buffHtml += ` <span style="font-size:0.8em; cursor:pointer;">(點擊查看)</span>`;
    ...
    updateEquipmentSlots() {
        ...
        const getDisplayHtml = (item) => {
            ...
            if (displayName.startsWith("undefined")) displayName = displayName.replace("undefined", "").trim();
    ...
    updateUI() {
        ...
        // 自動存檔
        if (!gameState.isLoading && player.hp > 0 && player.class && gameState.phase !== 'hub') {
    ...
    showBuffModal() {
        ...
        const createItem = (name, icon, turns, desc, type) => {
            ...
            let html = `<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:5px;">
                ...
                ${turns ? `<span style="font-size:0.8em; color:#aaa; background:#222; padding:2px 6px; border-radius:10px;">剩餘 ${turns} 回合</span>` : '<span style="font-size:0.8em; color:#aaa;">常駐</span>'}
            </div>
            <div style="color:#ccc; font-size:0.9em; line-height:1.4;">${desc || '無描述'}</div>`;
        ...
        // 3. 哈比威脅
        if (window.GameState.harpyThreat) {
            container.appendChild(createItem("來自空中的威脅", "🦅", null, "受到哈比的精準盯防！<br>逃跑率降低 40%，且逃跑失敗時會被偷竊物品！", "harpy"));
        }
    ...
    renderBlacksmithUI() {
        ...
        if (!window.Player.inventory || !window.Player.inventory.equipment || window.Player.inventory.equipment.length === 0) {
            ...
            window.Player.inventory.equipment.forEach((item, idx) => {
                ...
                // [Fix] Handle corrupted name "undefined [xxx]"
                let itemName = item.name || "未命名物品";
                if (itemName.startsWith("undefined")) {
                    itemName = itemName.replace("undefined", "").trim();
                }

                const desc = window.ItemSystem.getItemDesc(item);
                const rarityConfig = CONFIG.rarityDisplay[item.rarity] || CONFIG.rarityDisplay['common'];
                const rarityColor = rarityConfig.color;
                const enhance = item.enhance || 0;
                const price = item.price || 0;
                const cost = Math.floor(price / 2);

                html += `<div class="merchant-item ${rarityColor}" style="position:relative; display:flex; flex-direction:column; justify-content:space-between;">
                    <div class="m-top">
    ...
    renderBlacksmithMaterialSelect(targetIdx, materials) {
        ...
        let targetName = targetItem.name || "未命名物品";
        if (targetName.startsWith("undefined")) {
            targetName = targetName.replace("undefined", "").trim();
        }

        const targetRarity = CONFIG.rarityDisplay[targetItem.rarity] || CONFIG.rarityDisplay['common'];

        let html = `
            <div style="display:flex; flex-direction:column; gap:15px; padding:5px;">
                <div style="background:#222; padding:12px 15px; border-radius:8px; border:1px solid #444; display:flex; justify-content:space-between; align-items:center;">
                    <span style="color:#aaa; font-size:0.9em;">目標裝備:</span>
                    <span class="${targetRarity.color}" style="font-weight:bold; font-size:1.1em;">${targetItem.icon || ''} ${targetName}</span>
                </div>

                <div style="font-size:0.85em; color:#888; margin-bottom:-5px; padding-left:5px;">請選擇一件裝備作為祭品 (將消耗):</div>

                <div style="max-height:350px; overflow-y:auto; border:1px solid #444; border-radius:8px; background:#1a1a1a;">
        `;

        if (materials.length === 0) {
            html += `<p style="color:#666; grid-column:span 2;">沒有符合條件的素材</p>`;
        }

        materials.forEach(mat => {
            const item = mat.item;
            let matName = item.name || "未命名";
            if (matName.startsWith("undefined")) {
                matName = matName.replace("undefined", "").trim();
            }

            const rarityConfig = CONFIG.rarityDisplay[item.rarity] || CONFIG.rarityDisplay['common'];
            const rarityColor = rarityConfig.color;
            let desc = window.ItemSystem.getItemDesc(item);
            if (desc.startsWith("undefined")) desc = desc.replace("undefined", "").trim();

            html += `<div onclick="window.Game.confirmEnhance(${targetIdx}, ${mat.index})" 
                         style="padding:15px; border-bottom:1px solid #333; cursor:pointer; display:flex; justify-content:space-between; align-items:center; transition:background 0.2s;"
                         onmouseover="this.style.background='#282828'" onmouseout="this.style.background='transparent'">
                <div style="flex:1;">
                    <div class="${rarityColor}" style="font-weight:bold; margin-bottom:6px; font-size:1em;">${item.icon || '📦'} ${matName}</div>
                    <div style="font-size:0.85em; color:#777;">${desc}</div>
                </div>
                <div style="margin-left:15px;">
                    <button class="btn" style="background:#2196f3; color:white; padding:6px 15px; font-size:0.9em; border-radius:4px; border:none; cursor:pointer;">選擇</button>
                </div>
            </div>`;
        });
        ...
        html += `
                ...
                <button onclick="window.UISystem.closeModal()" class="btn-cancel" style="width:100%; padding:12px; margin-top:10px; font-size:1em;">取消</button>
            </div>
        `;

        this.showModal("⚒️ 選擇強化素材", html);
        // Remove default close button to use custom full-width one
        setTimeout(() => {
            const overlays = document.querySelectorAll('.modal-overlay');
            if (overlays.length > 0) {
                const current = overlays[overlays.length - 1];
                const actions = current.querySelector('.modal-actions');
    ...
    showDemonBuffSelection(skillLv, isEnhanced, pickCount, turns, callback) {
        ...
        titleEl.textContent = "😈 來自深淵的餽贈";

        // 1. 篩選可用 Buff (包含 demon 和 demon_enhanced)
        const allBuffs = Object.values(CONFIG.buffs);
        let candidates = allBuffs.filter(b => b.type === 'demon' || b.type === 'demon_enhanced');

        // 如果不是強化版，排除 "_plus" 結尾的 Buff
        if (!isEnhanced) {
            candidates = candidates.filter(b => !b.id.includes('_plus'));
        } else {
            // 強化版：只顯示強化版 Buff (_plus)
            candidates = candidates.filter(b => b.id.includes('_plus'));
        }

        // 2. 顯示所有選項 (不做隨機限制，讓玩家有更多選擇)
    ...
    _renderDemonSelection() {
        ...
        let html = `<p style="color:#ba68c8; font-size:0.9em; margin-bottom:10px; text-align:center;">
            請選擇 <span style="color:#fff; font-weight:bold;">${state.pickCount}</span> 個賜福 
            <span style="color:#aaa;">(持續 ${state.turns} 回合)</span>
        </p>`;
        ...
        state.options.forEach(buff => {
            ...
            <div onclick="window.UISystem.handleDemonObjClick('${buff.id}')"
                ...
                ${isSelected ? '<span style="color:#69f0ae;">✔</span>' : ''}
            </div>`;
        });
        html += `</div>`;
    ...
    showClassConfirm(classId) {
        ...
        document.getElementById('class-confirm-hp').innerText = cls.stats.hp + bonusHp; // 顯示總和
        document.getElementById('class-confirm-sp').innerText = cls.stats.sp;
        document.getElementById('class-confirm-atk').innerText = cls.stats.atk + bonusAtk; // 顯示總和
    ...
    renderAlchemyUI() {
        ...
        // [Fix] 保存當前折疊狀態
        const potionsGroup = document.getElementById('group-alchemy_potions');
        const convertGroup = document.getElementById('group-alchemy_convert');
        const isPotionsOpen = potionsGroup ? potionsGroup.style.display !== 'none' : false;
        const isConvertOpen = convertGroup ? convertGroup.style.display !== 'none' : false;

        content.innerHTML = '';
        const recipes = CONFIG.recipes;

        // [Mod] 建立折疊群組容器
        const groupId = 'alchemy_potions';
        const groupColor = '#9c27b0'; // 紫色主題

        let html = `
            <div style="border: 1px solid ${groupColor}; border-radius: 10px; overflow: hidden; background: #222; margin-bottom: 20px;">
                <div onclick="window.UISystem.toggleAlchemyGroup('${groupId}')" 
                     style="background: rgba(156, 39, 176, 0.15); padding: 10px 15px; border-bottom: 1px solid ${groupColor}; display:flex; justify-content:space-between; align-items:center; cursor:pointer; user-select: none;">
                    <h3 style="margin:0; color: ${groupColor}; font-size: 1.1em;">
                        密藥&卷軸 
                        <span style="font-size:0.8em; color:#bbb; font-weight:normal; margin-left:8px;">(消耗品製作)</span>
                    </h3>
                    <span id="arrow-${groupId}" style="color:${groupColor}; font-size: 1.2em; transition: transform 0.2s; transform: ${isPotionsOpen ? 'rotate(90deg)' : 'rotate(0deg)'};">${isPotionsOpen ? '▼' : '▶'}</span>
                </div>
                <div id="group-${groupId}" style="display: ${isPotionsOpen ? 'grid' : 'none'}; padding: 15px; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 15px;">
        `;
        ...
        Object.values(recipes).forEach(recipe => {
            // 每個配方的卡片
            html += `
                <div class="recipe-item" style="background:#333; padding:15px; border-radius:8px; display:flex; flex-direction:column; gap:10px; border:1px solid #555;">
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <div style="color:#ccc; font-size:0.9em; min-height: 40px;">${recipe.desc}</div>
            ...
            recipe.materials.forEach(mat => {
                ...
                html += `
                    <div style="background:#222; padding:5px 10px; border-radius:4px; border:1px solid ${hasEnough ? '#4caf50' : '#f44336'}; color:${hasEnough ? '#fff' : '#f44336'}; font-size:0.9em;">
                        ${mat.item}: ${owned} / ${mat.count}
                    </div>
                `;
            });

            html += `</div>`; // End materials

            // 合成按鈕
            html += `
                <button onclick="ItemSystem.craftItem('${recipe.id}')" 
                    ${canCraft ? '' : 'disabled'}
                    style="margin-top:auto; padding:10px; background:${canCraft ? 'linear-gradient(45deg, #673ab7, #9c27b0)' : '#444'}; color:${canCraft ? 'white' : '#888'}; border:none; border-radius:5px; cursor:${canCraft ? 'pointer' : 'not-allowed'}; font-weight:bold;">
                    ${canCraft ? '⚗️ 合成' : '素材不足'}
                </button>
            `;

            html += `</div>`; // End recipe-item
        });

        html += `   </div>
                </div>`; // End group body & container

        // [New] 建立素材轉化群組容器
        const convertGroupId = 'alchemy_convert';
        const convertColor = '#00bcd4'; // 藍色主題 (轉化)

        let convertHtml = `
            <div style="border: 1px solid ${convertColor}; border-radius: 10px; overflow: hidden; background: #222; margin-bottom: 20px;">
                <div onclick="window.UISystem.toggleAlchemyGroup('${convertGroupId}')" 
                     style="background: rgba(0, 188, 212, 0.15); padding: 10px 15px; border-bottom: 1px solid ${convertColor}; display:flex; justify-content:space-between; align-items:center; cursor:pointer; user-select: none;">
                    <h3 style="margin:0; color: ${convertColor}; font-size: 1.1em;">
                        素材轉化 
                        <span style="font-size:0.8em; color:#bbb; font-weight:normal; margin-left:8px;">(低階換高階 10:1 / 3:1)</span>
                    </h3>
                    <span id="arrow-${convertGroupId}" style="color:${convertColor}; font-size: 1.2em; transition: transform 0.2s; transform: ${isConvertOpen ? 'rotate(90deg)' : 'rotate(0deg)'};">${isConvertOpen ? '▼' : '▶'}</span>
                </div>
                <div id="group-${convertGroupId}" style="display: ${isConvertOpen ? 'grid' : 'none'}; padding: 15px; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 15px;">
        `;

        // 遍歷 museumSets (系列)
        if (CONFIG.museumSets) {
            CONFIG.museumSets.forEach(set => {
                const items = set.items; // [0]=T1, [1]=T2, [2]=T3
                if (!items || items.length < 3) return;

                const t1 = items[0].item;
                const t2 = items[1].item;
                const t3 = items[2].item;

                // 獲取數量和圖示
                const c1 = window.ItemSystem.getItemCount(t1, true);
                const c2 = window.ItemSystem.getItemCount(t2, true);
                const c3 = window.ItemSystem.getItemCount(t3, true);

                const icon1 = CONFIG.lootData[t1]?.icon || '📦';
                const icon2 = CONFIG.lootData[t2]?.icon || '📦';
                const icon3 = CONFIG.lootData[t3]?.icon || '📦';

                convertHtml += `
                    <div style="background:#2bd0d011; border:1px solid #00bcd444; border-radius:8px; padding:10px; display:flex; flex-direction:column; gap:10px;">
                        <div style="font-weight:bold; color:#00bcd4; border-bottom:1px solid #00bcd444; padding-bottom:5px;">${set.name.replace('收藏', '系列')}</div>
                        
                        <!-- T1 <-> T2 -->
                        <div style="display:flex; justify-content:space-between; align-items:center; font-size:0.9em;">
                            <div style="display:flex; flex-direction:column; align-items:center; width:30%;">
                                <div style="color:${c1 >= 10 ? '#fff' : '#aaa'}">${icon1} ${t1}</div>
                                <div style="font-size:0.8em; color:#aaa;">(持: ${c1})</div>
                            </div>
                            <div style="display:flex; flex-direction:column; gap:5px; align-items:center; width:40%;">
                                <button onclick="ItemSystem.convertMaterial('${set.id}', 0, 1)" class="btn-tiny" ${c1 >= 10 ? '' : 'disabled'} style="font-size:0.8em; padding:2px 5px; cursor:${c1 >= 10 ? 'pointer' : 'not-allowed'}; background:${c1 >= 10 ? '#4caf50' : '#333'}; color:white; border:none; border-radius:3px;">
                                    合成 (10➡1)
                                </button>
                                <button onclick="ItemSystem.convertMaterial('${set.id}', 1, 0)" class="btn-tiny" ${c2 >= 1 ? '' : 'disabled'} style="font-size:0.8em; padding:2px 5px; cursor:${c2 >= 1 ? 'pointer' : 'not-allowed'}; background:${c2 >= 1 ? '#f44336' : '#333'}; color:white; border:none; border-radius:3px;">
                                    分解 (1➡10)
                                </button>
                            </div>
                            <div style="display:flex; flex-direction:column; align-items:center; width:30%;">
                                <div style="color:${c2 >= 1 ? '#fff' : '#aaa'}">${icon2} ${t2}</div>
                                <div style="font-size:0.8em; color:#aaa;">(持: ${c2})</div>
                            </div>
                        </div>

                        <div style="height:1px; background:#ffffff22;"></div>

                        <!-- T2 <-> T3 -->
                        <div style="display:flex; justify-content:space-between; align-items:center; font-size:0.9em;">
                            <div style="display:flex; flex-direction:column; align-items:center; width:30%;">
                                <div style="color:${c2 >= 3 ? '#fff' : '#aaa'}">${icon2} ${t2}</div>
                                <div style="font-size:0.8em; color:#aaa;">(持: ${c2})</div>
                            </div>
                            <div style="display:flex; flex-direction:column; gap:5px; align-items:center; width:40%;">
                                <button onclick="ItemSystem.convertMaterial('${set.id}', 1, 2)" class="btn-tiny" ${c2 >= 3 ? '' : 'disabled'} style="font-size:0.8em; padding:2px 5px; cursor:${c2 >= 3 ? 'pointer' : 'not-allowed'}; background:${c2 >= 3 ? '#4caf50' : '#333'}; color:white; border:none; border-radius:3px;">
                                    合成 (3➡1)
                                </button>
                                <button onclick="ItemSystem.convertMaterial('${set.id}', 2, 1)" class="btn-tiny" ${c3 >= 1 ? '' : 'disabled'} style="font-size:0.8em; padding:2px 5px; cursor:${c3 >= 1 ? 'pointer' : 'not-allowed'}; background:${c3 >= 1 ? '#f44336' : '#333'}; color:white; border:none; border-radius:3px;">
                                    分解 (1➡3)
                                </button>
                            </div>
                            <div style="display:flex; flex-direction:column; align-items:center; width:30%;">
                                <div style="color:${c3 >= 1 ? '#fff' : '#aaa'}">${icon3} ${t3}</div>
                                <div style="font-size:0.8em; color:#aaa;">(持: ${c3})</div>
                            </div>
                        </div>
                    </div>
                `;
        ...
        convertHtml += `   </div>
                </div>`; // End convert group

        // 將兩個 HTML 區塊合併 (先 Potions, 後 Convert)
        content.innerHTML = html + convertHtml;
    },

    /**
     * [New] 切換煉金術群組顯示狀態
     */
    toggleAlchemyGroup(groupId) {
    ...
    /**
     * [New] 顯示通用模態窗
     * @param {string} title - 標題 (可為 null)
     * @param {string} content - HTML 內容
     * @param {boolean} autoClose - 是否點擊背景關閉
     */
    showModal(title, content, autoClose = true) {
        ...
        // 如果內容已經包含 modal-content 結構，直接使用
            ...
            div.innerHTML = `
                <div class="modal-content">
                    ...
                    <div class="modal-actions">
                        <button class="action-btn secondary" onclick="window.UISystem.closeModal()">關閉</button>
                    </div>
                </div>
            `;
        }
        ...
        // 強制 reflow 並添加 active class 以觸發動畫 (如果有的話)
    ...
    showBossInfo() {
        ...
        // 根據名稱判斷
        if (enemy.name.includes("哥布林族長")) {
            content = `
                <div style="text-align:center; margin-bottom:15px;">
                    <img src="${enemy.image}" alt="${enemy.name}" style="width:120px; height:120px; object-fit:contain; margin-bottom:10px; filter: drop-shadow(0 0 10px rgba(255,255,255,0.2));">
                    <h3 style="color:#ff9800;">哥布林族長</h3>
                    <div style="color:#aaa; font-size:0.9em;">Depth 50 | 貪婪的首領</div>
                </div>
                <div style="background:#333; padding:10px; border-radius:5px; margin-bottom:10px;">
                    <h4 style="color:#ffeb3b; margin:0 0 5px 0;">👑 被動：金幣收藏家</h4>
                    <p style="margin:0; font-size:0.9em;">玩家每擁有 10G，最大生命值增加 0.1%。</p>
                </div>
                <div style="background:#333; padding:10px; border-radius:5px; margin-bottom:10px;">
                    <h4 style="color:#f44336; margin:0 0 5px 0;">⚔️ 技能：覬覦財富 (CD 5)</h4>
                    <p style="margin:0; font-size:0.9em;">偷取玩家 15% 金幣，並造成傷害。<br><span style="color:#81d4fa;">弱點：因分心，Boss 會受到等同於被偷金幣量的傷害。</span></p>
                </div>
                <div style="background:#333; padding:10px; border-radius:5px;">
                    <h4 style="color:#9c27b0; margin:0 0 5px 0;">☠️ 技能：劇毒塗藥 (CD 6)</h4>
                    <p style="margin:0; font-size:0.9em;">80%機率 施加「劇毒」Debuff (每回合扣 8 HP)。<br>「劇毒」有 30% 機率施加「虛弱」(攻擊力 -30%)。</p>
                </div>
            `;
        } else if (enemy.name.includes("史萊姆之王")) {
            content = `
                <div style="text-align:center; margin-bottom:15px;">
                    <img src="${enemy.image}" alt="${enemy.name}" style="width:120px; height:120px; object-fit:contain; margin-bottom:10px; filter: drop-shadow(0 0 10px rgba(255,255,255,0.2));">
                    <h3 style="color:#2196f3;">史萊姆之王</h3>
                    <div style="color:#aaa; font-size:0.9em;">Depth 100 | 黏液之王</div>
                </div>
                <div style="background:#333; padding:10px; border-radius:5px; margin-bottom:10px;">
                    <h4 style="color:#4caf50; margin:0 0 5px 0;">💚 被動：再生</h4>
                    <p style="margin:0; font-size:0.9em;">每次施放技能時，回復自身 100 HP。</p>
                </div>
                <div style="background:#333; padding:10px; border-radius:5px; margin-bottom:10px;">
                    <h4 style="color:#f44336; margin:0 0 5px 0;">💥 技能：爆炸分裂 (CD 6)</h4>
                    <p style="margin:0; font-size:0.9em;">蓄力 2 回合。對玩家造成 <span style="color:#ff5252;">當前生命 8%</span> 的傷害。<br><span style="color:#2196f3;">干擾：蓄力期間每攻擊一次，傷害倍率減少 1.5%。</span></p>
                </div>
                <div style="background:#333; padding:10px; border-radius:5px;">
                    <h4 style="color:#ff9800; margin:0 0 5px 0;">💧 技能：黏液噴灑 (CD 6)</h4>
                    <p style="margin:0; font-size:0.9em;">造成傷害並施加「黏黏的」Debuff (5回合)。<br>每回合扣 10 HP，且有 30% 機率無法行動。</p>
                </div>
            `;
        } else {
            content = `<p style="text-align:center; padding:20px; color:#aaa;">尚無此首領的詳細資訊。</p>`;
        }

        const modal = document.getElementById('boss-info-modal');
        const contentEl = document.getElementById('boss-info-content');
        if (modal && contentEl) {
