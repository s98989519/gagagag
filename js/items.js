...
var ItemSystem = {
    ...
    useItem(index, category) {
        ...
        if (item.name.includes("藥水") || item.name.includes("藥劑")) {
            ...
            if (window.Player.buff && window.Player.buff.id === 'angel_vitality') {
                ...
                heal += bonus;
                bonusText = ` <span style="font-size:0.8em; color:#69f0ae">(天使的活力 +${bonus})</span>`;
            }
    ...
    craftItem(recipeId) {
        ...
        for (let mat of recipe.materials) {
            if (this.getItemCount(mat.item) < mat.count) {
        ...
        // 2. 扣除素材
        for (let mat of recipe.materials) {
            this.removeItems(mat.item, mat.count);
        }

        // 3. 給予獎勵
        if (recipe.resultType === 'buff') {
            const buff = Object.values(CONFIG.buffs).find(b => b.id === recipe.buffId);
            if (buff) {
                window.Player.buff = buff;
                window.Game.showFloatingText(`獲得 Buff: ${buff.name}`, "#ffd700");
                window.UISystem.showToast(`煉金成功！獲得 ${buff.name}`, "success");
            }
        } else if (recipe.resultType === 'consumable' || recipe.resultType === 'item') {
            // 簡化處理：直接生成物品
            const newItem = {
                name: recipe.name,
                icon: recipe.icon,
            ...
            window.UISystem.showToast(`煉金成功！獲得 ${recipe.name}`, "success");
        }
