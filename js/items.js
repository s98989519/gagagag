...
var ItemSystem = {
    ...
    generateAffixes(item) {
        ...
        if (Math.random() < 0.3) {
            ...
            if (affix && affix.name) {
                item.prefix = key;
                item.name = `${affix.name} ${item.name || "未命名"}`;
        ...
        if (Math.random() < 0.3) {
            ...
            if (validSuffixes.length > 0) {
                ...
                if (affix && affix.name) {
                    item.suffix = key;
                    item.name = `${item.name || "未命名"}${affix.name}`;
                    item.desc = (item.desc || "") + ` [${affix.desc}]`;
                }
            }
        }
    ...
    addItemToInventory(item, render = true) {
        ...
        // [Fix] 傳說物品特效觸發導致移出 global check outside，改為檢查本局是否首次獲得
     ...
     * @param {string} name - 物品名稱
     * @param {number} count - 數量
     * @param {boolean} fromWarehouse - 是否優先從倉庫移除
     */
    removeItems(name, count, fromWarehouse = false) {
        let remaining = count;

        // 1. 若指定從倉庫移除，先扣倉庫
        if (fromWarehouse && window.Player.warehouse && window.Player.warehouse[name]) {
            const take = Math.min(window.Player.warehouse[name], remaining);
            window.Player.warehouse[name] -= take;
            remaining -= take;
            if (window.Player.warehouse[name] <= 0) delete window.Player.warehouse[name];
        }
        ...
        for (const cat of categories) {
            ...
            const inv = window.Player.inventory[cat];
            if (!inv) continue;
    ...
    useItem(index, category) {
        ...
        // [New] 輪迴沙漏
        if (item.id === 'hourglass' || item.name === '輪迴沙漏') {
            if (window.Game.showRebirthModal) {
                window.Game.showRebirthModal();
            } else {
                console.error("Game.showRebirthModal is not defined!");
            }
            return;
        }
        ...
        if (item.name.includes("藥水") || item.name.includes("藥劑")) {
        ...
        } else if (item.effect && item.effect.type === 'heal_percent') {
            // [New] 百分比回復類型（如罐裝泉水）
            const healPercent = item.effect.val;
            let heal = Math.floor(window.Player.maxHp * healPercent);
            let bonusText = "";

            // 天使的活力：藥水效果 +50%
            if (window.Player.buff && window.Player.buff.id === 'angel_vitality') {
                const bonus = Math.floor(heal * 0.5);
                heal += bonus;
                bonusText += ` <span style="font-size:0.8em; color:#69f0ae">(天使 +${bonus})</span>`;
            }
            ...
            // 符文：治癒之風 (+20%)
            if (window.Player.unlockedRunes && window.Player.unlockedRunes.includes('strong_heal')) {
                const bonus = Math.floor(heal * 0.2);
                heal += bonus;
                bonusText += ` <span style="font-size:0.8em; color:#69f0ae">(符文 +${bonus})</span>`;
            }
            ...
            const msg = `
                <div style="text-align:left; font-size:1.1em; line-height:1.6;">
                    <p><strong>使用物品:</strong> <span style="color:#69f0ae">${item.name}</span></p>
                    <p><strong>回復量:</strong> <span style="color:#69f0ae">+${heal}</span> (${Math.floor(healPercent * 100)}% 最大生命)${bonusText}</p>
                    <hr style="border-color:#444; margin:10px 0;">
                    <p><strong>生命值變化:</strong></p>
                    <p style="padding-left:15px;">當前: <span style="color:${currentHp < maxHp * 0.3 ? '#ff5252' : '#fff'}">${currentHp}</span> / ${maxHp}</p>
                    <p style="padding-left:15px;">使用後: <span style="color:#69f0ae">${predictedHp}</span> / ${maxHp}</p>
    ...
    /**
     * [New] 素材轉化
     * @param {string} seriesId - 系列ID (對應 museumSets)
     * @param {number} tierFrom - 來源階級 (0:T1, 1:T2, 2:T3)
     * @param {number} tierTo - 目標階級
     */
    convertMaterial(seriesId, tierFrom, tierTo) {
        const set = CONFIG.museumSets.find(s => s.id === seriesId);
        if (!set) return;

        const srcItemName = set.items[tierFrom].item;
        const targetItemName = set.items[tierTo].item;

        // 定義比率
        let cost = 0;
        let gain = 1;

        if (tierFrom === 0 && tierTo === 1) cost = 10; // T1 -> T2 (10:1)
        else if (tierFrom === 1 && tierTo === 2) cost = 3;  // T2 -> T3 (3:1)
        else if (tierFrom === 1 && tierTo === 0) { cost = 1; gain = 10; } // T2 -> T1 (1:10)
        else if (tierFrom === 2 && tierTo === 1) { cost = 1; gain = 3; }  // T3 -> T2 (1:3)
        else return;

        // 檢查數量 (優先檢查倉庫)
        const owned = this.getItemCount(srcItemName, true);

        if (owned >= cost) {
            // 執行轉換
            this.removeItems(srcItemName, cost);
            this.addItemToWarehouse(targetItemName, gain);

            // 播放音效 (如果有的話，這裡暫用合成音效概念)
            if (window.UISystem) {
                window.UISystem.showToast(`轉化成功：失去 ${cost} ${srcItemName}，獲得 ${gain} ${targetItemName}`);
                if (window.AudioSystem) window.AudioSystem.playSFX('craft');
                window.UISystem.renderAlchemyUI(); // 重新渲染介面
            }
        } else {
            if (window.UISystem) window.UISystem.showToast('素材不足！', 'error');
        }
    }
};
