...
const EventSystem = {
    ...
    triggerHarpy() {
        ...
        (window.Player.equipment.accessories || []).forEach(acc => {
            if (!acc) return;
            if (acc.id === 'acc_orc_1') repelBonus += 0.10;
            if (acc.id === 'acc_orc_2') repelBonus += 0.30;
            if (acc.id === 'acc_orc_3') repelBonus += 1.00; // 必定擊退
        });

        if (outcome < (0.2 + repelBonus)) {
            window.Player.gold += 500;
            window.Game.showFloatingText("+500 G", "#ffd700");
            desc = "你成功擊退了哈比，撿到了它掉落的 <span class='gold-text'>500 G</span>！";
            if (repelBonus > 0) {
                desc += "<br><span style='color:#69f0ae'>(飾品效果觸發)</span>";
            }
        } else {
