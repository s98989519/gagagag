...
const AudioSystem = {
    ...
    setBGMVolume(val) {
        ...
        if (v > 1) v /= 100; // 兼容 0-100 輸入
        this.bgmVolume = v;
        this.updateGain();
        localStorage.setItem('bgm_volume', this.bgmVolume);

        // 更新 UI 顯示
        const text = document.getElementById('bgm-volume-value'); // 更新為 index.html 中的 ID
        if (text) text.innerText = `${Math.round(this.bgmVolume * 100)}%`;

        // 同步更新舊的 ID 以防萬一
        const oldText = document.getElementById('bgm-vol-text');
        if (oldText) oldText.innerText = `${Math.round(this.bgmVolume * 100)}%`;
    },
    ...
    setSFXVolume(val) {
        ...
        // 更新 UI 顯示
        const text = document.getElementById('sfx-volume-value'); // 更新為 index.html 中的 ID
        if (text) text.innerText = `${Math.round(this.sfxVolume * 100)}%`;

        // 同步更新舊的 ID 以防萬一
        const oldText = document.getElementById('sfx-vol-text');
        if (oldText) oldText.innerText = `${Math.round(this.sfxVolume * 100)}%`;
    },
    ...
    updateVolumeUI() {
        ...
        if (bgmSlider) bgmSlider.value = this.bgmVolume * 100;
        if (sfxSlider) sfxSlider.value = this.sfxVolume * 100;

        const bgmText = document.getElementById('bgm-volume-value');
        if (bgmText) bgmText.innerText = `${Math.round(this.bgmVolume * 100)}%`;

        const sfxText = document.getElementById('sfx-volume-value');
