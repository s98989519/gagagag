...
const CombatSystem = {
    ...
    playerUltimate() {
        ...
        if (player.class === 'knight') {
            ...
            // 暗影一擊 Scale: 1.5, 1.7, 2.0, 2.3, 2.5, 3.0
            const dmgMap = { 1: 1.5, 2: 1.7, 3: 2.0, 4: 2.3, 5: 2.5, 6: 3.0 };
            const evaMap = { 1: 10, 2: 15, 3: 18, 4: 18, 5: 20, 6: 20 };
            const turnMap = { 1: 3, 2: 3, 3: 3, 4: 4, 5: 4, 6: 4 };

            ultDmg = baseDmgCalc(dmgMap[skillLv] || 1.5);
            logHtml += `🗡️ [暗影一擊 Lv.${skillLv}] 還沒看清就倒下了！<br>`;

            if (!player.extraBuffs) player.extraBuffs = [];
            player.extraBuffs.push({
                id: 'thief_evasion',
                name: '殘影',
                evasion: evaMap[skillLv] || 10,
                icon: '💨',
                desc: `閃避率 +${evaMap[skillLv] || 10}%`,
                turns: turnMap[skillLv] || 3,
                newThisTurn: true
            });
            logHtml += `<span style="color:#69f0ae;">💨 獲得殘影 (+${evaMap[skillLv] || 10}% 閃避)</span><br>`;

            // Lv3+ 下次必爆
            if (skillLv >= 3) {
                // To implement "Next hit Crit", we use a hidden buff or state
                player.extraBuffs.push({ id: 'thief_next_crit', name: '預謀', type: 'special', icon: '🎯', desc: '下次攻擊必定暴擊', turns: 99, newThisTurn: true });
                logHtml += `<span style="color:#ffeb3b;">🎯 下次攻擊必定暴擊！</span><br>`;
            }
            // Lv6 閃避後必爆 (Need to handle in handleMonsterAttack evasion check)
            if (skillLv >= 6) {
                player.extraBuffs.push({ id: 'thief_dodge_crit_buff', name: '瞬影', type: 'special', icon: '🌀', desc: '閃避成功後獲得必定暴擊', turns: turnMap[skillLv] || 4, newThisTurn: true });
            }
