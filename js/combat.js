...
const CombatSystem = {
    ...
    getWeightedMonster() {
        ...
        if (window.Player.depth < 50) {
            // 50層以下：大幅提升史萊姆、哥布林、狂狼的機率
            CONFIG.monsters.forEach((m, idx) => {
                let tempM = { ...m };
                // idx 0: 史萊姆, 1: 哥布林, 2: 狂狼
                if (idx <= 2) {
                    tempM.weight = 30; // 高機率
                } else {
                    tempM.weight = 1;  // 低機率
    ...
    createEnemy(baseMonster, tier, checkTrueForm) {
        ...
        if (window.Player.depth >= 300) {
            ...
            if (window.Player.depth < 600) {
                // 階段1: 300~599層 (每50層+10%)
                const chunks = Math.floor((window.Player.depth - 300) / 50) + 1;
                bonus = chunks * 0.10;
            } else {
                // 階段2: 600層+
                // 先加上 300-599 的滿額加成 (300,350,400,450,500,550 共6階 = 60%)
                bonus = 0.60;

                // 加上 600+ 的加成 (每50層+12%)
                const chunks = Math.floor((window.Player.depth - 600) / 50) + 1;
                bonus += chunks * 0.12;
            }
        ...
        if (tier === "elite") {
            ...
            atkMul *= 2;
            namePrefix += "首領 ";
            penetration = 0.25; // BOSS 獲得 25% 穿透
        }
    ...
    executeMonsterAttack(enemy) {
        ...
        let damageReduction = totalDef / (totalDef + 50); // DR 公式: Def / (Def + 50)

        // 應用 Boss 穿透機制 (Penetration)
        if (enemy.penetration > 0) {
            // 實際減傷率 = 原始 DR * (1 - 穿透率)
            damageReduction *= (1 - enemy.penetration);
            log += `<span class='damage-text'>[${enemy.name}]</span> 穿透了你 ${Math.round(enemy.penetration * 100)}% 的防禦！<br>`;
        }
        ...
        // 計算最終傷害
        mDmg = Math.floor(mDmg * (1 - damageReduction));

        // 最低傷害機制：至少受到 1 點傷害 (或 5% 攻擊力，防止 Boss 被完全免疫)
        const minDmg = Math.max(1, Math.floor(currentAtk * 0.05));
        mDmg = Math.max(minDmg, mDmg);
