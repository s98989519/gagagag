...
const Game = {
    ...
    renderWorkbench() {
        ...
        let html = `
            <div style="text-align:center; padding:20px;">
                <!-- 標題已移至模態框 Header -->
                <p style="color:#aaa; margin-bottom: 20px;">消耗倉庫素材製作強力飾品</p>
        ...
        groups.forEach(group => {
            ...
            recipes.forEach(recipe => {
                ...
                recipe.materials.forEach(mat => {
                    const owned = window.ItemSystem.getItemCount(mat.item, true);
                    const enough = owned >= mat.count;
                    if (!enough) hasMaterials = false;
    ...
    craftItem(recipeId) {
        ...
        // 扣除素材
        for (let mat of recipe.materials) {
            window.ItemSystem.removeItems(mat.item, mat.count, true);
        }
    ...
    recalcStats() {
        ...
        // 飾品加成 (Golem: HP, Demon: HP) - 最後加算，避免被倍率過度放大
        (Player.equipment.accessories || []).forEach(acc => {
            if (!acc) return;
            // Golem Series
    ...
    withdrawFromWarehouse(itemName, count = 1) {
        ...
        } else if (['weapon', 'armor', 'shield', 'accessory'].includes(newItem.type)) {
