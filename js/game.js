...
const Player = {
    ...
    // [CRITICAL FIX] 訓練場與符文系統初始化
    trainingFacilityLevel: 1,
    trainingLevels: { hp: 0, atk: 0 },
    unlockedRunes: [],        // 已解鎖的符文
    shardsCollected: [],      // 已收集的碎片
    skillLevels: {},          // 技能等級
    nextRunBuffs: [],         // 下局 Buff
...
const Game = {
    ...
    init() {
        ...
        // [CRITICAL FIX] 在所有初始化完成後重新計算屬性
        // 確保符文加成（閃避率、暴擊率等）正確套用
        this.recalcStats();
    },
     ...
     * 放棄探險
     */
    giveUpAdventure(confirmed = false) {
        // 如果尚未確認，先顯示確認對話框
        if (!confirmed) {
            const currentEp = Player.epGainedThisRun || 0;
            const willGet = Math.floor(currentEp / 2);
            const willLose = currentEp - willGet;

            const message = `
                <div style="text-align: left; padding: 10px;">
                    <div style="background: rgba(255, 235, 59, 0.1); border: 1px solid rgba(255, 235, 59, 0.3); border-radius: 8px; padding: 15px; margin-bottom: 15px;">
                        <div style="font-size: 1.1em; color: #ffeb3b; font-weight: bold; margin-bottom: 10px;">📊 本局收益結算</div>
                        <div style="color: #e0e0e0; margin: 5px 0;">• 本局累積 EP：<span style="color: #fff; font-weight: bold;">${currentEp}</span></div>
                        <div style="color: #4caf50; margin: 5px 0;">✅ 將保留：<span style="color: #4caf50; font-weight: bold;">${willGet} EP</span> (50%)</div>
                        <div style="color: #f44336; margin: 5px 0;">❌ 將損失：<span style="color: #f44336; font-weight: bold;">${willLose} EP</span> (50%)</div>
                    </div>
                    <div style="margin-top: 15px; padding: 10px; background: rgba(255, 152, 0, 0.1); border-radius: 6px; border: 1px solid rgba(255, 152, 0, 0.3);">
                        <div style="color: #ff9800; font-size: 0.95em; text-align: center;">
                            ⚠️ 確定要放棄本次探險嗎？
                        </div>
                    </div>
                </div>
            ...
            window.UISystem.showConfirmModal(
                "⚠️ 放棄探險確認",
                message,
                () => this.giveUpAdventure(true), // 確認後執行
                true, // 顯示取消按鈕
                true, // 顯示確認按鈕
                "確定放棄",
                "取消"
        ...
        // [CRITICAL FIX] 保存永久數據（EP、博物館、訓練場等）
        // 必須在 location.reload() 之前調用，否則所有永久數據會遺失！
        this.savePersistentData();
    ...
    unlockRune(runeId) {
        ...
        console.log(`[DEBUG] 符文已解鎖: ${runeId}, Player.unlockedRunes:`, Player.unlockedRunes);

        this.savePersistentData();
        console.log("[DEBUG] 永久數據已保存");

        // [CRITICAL FIX] 立即重新計算屬性，讓符文效果生效
        this.recalcStats();
        console.log("[DEBUG] 屬性已重新計算");
    ...
    closePortableWorkbench() {
        ...
        if (GameState.currentEnemy) {
            ...
            // [Fix] 如果在戰鬥中，必須恢復戰鬥按鈕
            if (window.CombatSystem && window.UISystem) {
                const fleeRate = window.CombatSystem.getFleeRate();
                const fleeText = `逃跑 (${Math.round(fleeRate * 100)}%)`;
                window.UISystem.setCombatButtons("戰鬥", "combatRound", fleeText, "flee", "playerDefend");
            }
            ...
            // 我們假設玩家是在非戰鬥狀態下使用的
            if (GameState.phase === 'workbench') {
                // 恢復為原本的事件狀態，如果不知道就設為 event_end 或 explore
                // 為安全起見，檢查是否有 currentEnemy
                GameState.phase = GameState.currentEnemy ? 'combat' : 'event_end';
            }
        }
                ...
                <div style="border: 1px solid ${color}; border-radius: 10px; overflow: hidden; background: #262626; text-align: left; box-shadow: 0 4px 6px rgba(0,0,0,0.3); display:flex; flex-direction:column;">
                    ...
                    <div style="padding: 15px; display:flex; flex-direction:column; flex-grow: 1;">
                        ...
                        <div style="margin-top:auto; padding-top:10px; text-align:center;">
                            ...
                            <!-- [New] 強化一覽按鈕 -->
                            <button onclick="window.Game.showSkillOverview('${classId}')" class="btn" 
                                style="width: auto; padding: 8px 15px; margin-left:10px; border-radius: 20px; border: 1px solid #777; background: rgba(255, 255, 255, 0.05); color: #ccc; font-weight:bold; transition:all 0.2s;">
                                📜 強化一覽
                            </button>
                        </div>
                    </div>
    ...
    showSkillUpgradeModal(classId) {
        ...
        try {
            ...
            const costConfig = config.costs[classId][currentLv];
    ...
    upgradeSkill(classId) {
        ...
        try {
            const config = window.CONFIG.hub.training.skillUpgrade;
            const currentLv = Player.skillLevels[classId] || 1;
            const costConfig = config.costs[classId][currentLv]; // Config for current level -> next level
     ...
     * [New] 顯示技能全等級效果一覽
     */
    showSkillOverview(classId) {
        const config = window.CONFIG.hub.training.skillUpgrade;
        const descriptions = config.descriptions[classId];
        const playerSkillLv = (window.Player.skillLevels && window.Player.skillLevels[classId]) || 1;

        if (!descriptions) {
            window.UISystem.showToast("找不到技能描述數據", "error");
            return;
        }
        ...
        for (let lv = 1; lv <= config.maxLevel; lv++) {
            const desc = descriptions[lv] || "未知效果";
            const isCurrent = (lv === playerSkillLv);
            const isNext = (lv === playerSkillLv + 1);
            ...
            html += `
                <div style="background: ${bg}; border: 1px solid ${border}; border-radius: 8px; padding: 10px; margin-bottom: 8px;">
                    ...
                    <div style="color: ${color}; font-size: 0.95em;">${desc}</div>
                </div>
            `;
        }
        ...
        const classNameMap = {
            'knight': '騎士', 'merchant': '商販', 'thief': '盜賊',
            'cultist': '惡魔信徒', 'scarecrow': '稻草人', 'ape': '人猿'
        };
        const className = classNameMap[classId] || classId;

        // Ensure showConfirmModal is called correctly
        // signature: showConfirmModal(title, message, onConfirm, showCancel = true, showConfirmBtn = true, confirmText = "確定", cancelText = "取消")
        window.UISystem.showConfirmModal(
            `${className} - 技能強化一覽`,
            html,
            null, // onConfirm
            false, // showCancel
            true, // showConfirmBtn
            "關閉" // confirmText acting as Close
        );
    },
    ...
    recalcStats() {
        ...
        (Player.equipment.accessories || []).forEach(acc => {
            if (!acc) return;
            if (acc.id === 'acc_ghost_1') bonusEvasion += 5; // +5%
            if (acc.id === 'acc_ghost_2') bonusEvasion += 10; // +10%
            if (acc.id === 'acc_ghost_3') bonusEvasion += 15; // +15%
        });

        // 符文加成
        let runeEvasion = 0;
        if (Player.unlockedRunes && Player.unlockedRunes.includes('evasion_boost')) {
            runeEvasion = CONFIG.runes.evasion_boost.effect.val;
            bonusEvasion += runeEvasion;
        }
        ...
        // [DEBUG] 輸出詳細資訊
        console.log("=== 閃避率計算詳細 ===");
        console.log("基礎閃避:", baseEvasion);
        console.log("符文加成:", runeEvasion, "(evasion_boost已解鎖:", Player.unlockedRunes?.includes('evasion_boost'), ")");
        console.log("飾品加成:", bonusEvasion - runeEvasion);
        console.log("總閃避率:", Player.evasion);
        console.log("Player.unlockedRunes:", Player.unlockedRunes);
        ...
        (Player.equipment.accessories || []).forEach(acc => {
            ...
            // Wolf Series
            if (acc.id === 'acc_wolf_1') bonusCrit += 3; // +3%
            if (acc.id === 'acc_wolf_2') bonusCrit += 5; // +5%
            if (acc.id === 'acc_wolf_3') bonusCrit += 10; // +10%
            // Demon Series
