...
const Game = {
    ...
    renderSkillTraining() {
        ...
        let html = `
            <div style="text-align:center; padding:20px;">
                <h2 style="color:#607d8b; margin-bottom:10px; font-size: 2em;">📚 技能特訓</h2>
        ...
        for (const [classId, classData] of Object.entries(window.CONFIG.classes)) {
            ...
            // [Fix] Use new description from config
            const skillDesc = config.descriptions[classId][currentLv] || classData.skill.desc;

            const isMax = currentLv >= config.maxLevel;

            html += `
                <div style="background: linear-gradient(135deg, #333 0%, #222 100%); border: 1px solid #555; border-radius: 10px; padding: 20px; text-align: center; position: relative; box-shadow: 0 4px 6px rgba(0,0,0,0.3);">
                    <div style="font-size: 2.5em; margin-bottom: 10px;">${classData.icon}</div>
                    <div style="font-size: 1.3em; color: #fff; margin-bottom: 5px; font-weight: bold;">${classData.name}</div>
                    <div style="font-size: 1em; color: #69f0ae; font-weight:bold; margin-bottom: 10px;">${skillName} <span style="background:#333; padding:2px 8px; border-radius:10px; font-size:0.8em;">Lv.${currentLv}</span></div>
                    <div style="font-size: 0.9em; color: #aaa; min-height: 40px;">${skillDesc}</div>
                    <button onclick="window.Game.showSkillUpgradeModal('${classId}')" class="btn" 
                        style="width:100%; margin-top: 15px; background: ${isMax ? '#555' : '#607d8b'};" ${isMax ? 'disabled' : ''}>
                        ${isMax ? '已達最高等級' : '點擊升級 ➤'}
                    </button>
                </div>
            `;
        ...
        html += `
                ...
                <button onclick="window.Game.enterHub()" class="btn" style="background:#555; margin-top:30px; padding: 10px 30px;">返回大廳</button>
            </div>
        `;
            ...
            const modalHtml = `
                <div class="modal-content" style="max-width: 450px; background: #222; border: 1px solid #555; padding: 20px; border-radius: 10px; box-shadow: 0 0 20px rgba(0,0,0,0.8);">
                    ...
                    <div style="margin: 15px 0; text-align: left; background:#1a1a1a; padding:15px; border-radius:8px; border: 1px solid #333;">
                        ...
                        <div style="margin-top:10px; font-size: 0.9em; color:#888; border-top:1px dashed #444; padding-top:10px;">
                            <span style="color:#69f0ae;">下一級效果:</span><br>
                            ${nextDesc}
                        </div>
                    </div>
    ...
    giveUpAdventure() {
        ...
        // 計算探索點數 (已改為擊殺獲取，此處僅保留最低限度的層數補償或完全移除)
        // 為了避免刷分，撤退不再給予額外 EP (除非挑戰模式)
        // [Refactor] EP 結算：放棄獲得 50%
        const currentRunEp = Player.epGainedThisRun || 0;
        ...
        const msg = `
            <div style="text-align:center; padding: 20px;">
                ...
                <div style="background:rgba(255,255,0,0.1); padding:10px; border-radius:5px; margin-bottom:10px;">
                    <p style="font-size:1.1em; color:#e0e0e0;">本局累積：${Player.epGainedThisRun || 0} EP</p>
                    <p style="font-size:1.3em; color:#ffeb3b; font-weight:bold; margin-top:5px;">🏳️ 撤退帶回：${savedEp} EP (50%)</p>
                </div>
    ...
    getCrit() {
        ...
        // 1. Buff 加成
        if (Player.buff) {
            if (Player.buff.id === 'angel_courage') crit += 20; // 天使的勇氣: +20%
            if (Player.buff.id === 'demon_enhance') crit = 50; // 惡魔的強化: 強制固定 50%
        }

        // 1.5 額外 Buff 加成
        if (window.Player.extraBuffs) {
            window.Player.extraBuffs.forEach(buff => {
                if (buff.crit) crit += buff.crit;
                // Specific ID logic - 檢查順序：Plus 優先
                if (buff.id === 'demon_enhance_plus') {
                    crit += 40; // +40% 爆擊率
                } else if (buff.id === 'demon_enhance') {
                    crit = 50; // 固定 50%
                }
            });
        }
    ...
    recalcStats() {
        ...
        let bonusCrit = 0;
        if (this.modifiers && this.modifiers.crit) {
            bonusCrit += Math.floor(this.modifiers.crit * 100); // deadly: 0.05 -> 5
        }
        // 飾品加成
        (Player.equipment.accessories || []).forEach(acc => {
            if (!acc) return;
            if (acc.crit) bonusCrit += acc.crit; // 假設飾品直接加整數? 需確認。Dragon set gives 5.
            // Dragon set reward: type 'crit_bonus', val 5.
            // Affix 'deadly': val 0.05.
            // Consistency needed. Assume acc.crit is integer if > 1.
        });

        // 套裝/博物館加成 (Crit)
        bonusCrit += this.getMuseumBonus('crit_bonus'); // val 5
    ...
    getDef() {
        // 如果 Player.def 未定義，則嘗試計算或返回 0
        return Player.def || 0;
    },

    getAtk() {
        return Player.atk || Math.floor((Player.baseAtk || 5) * (1 + (window.Game.modifiers && window.Game.modifiers.atk || 0)));
    },
