        ...
        if (hasSave) {
            ...
            contentHtml = `
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:20px; max-width:600px; margin:0 auto;">
                    ...
                    <button onclick="window.Game.renderLibrary()" class="btn" style="background:#9c27b0; padding: 20px; font-size: 1.2em;">
                        📚 圖書館<br><span style="font-size:0.8em">圖鑑與 Buff</span>
                    </button>
                    <button onclick="window.Game.renderChallengeAltar()" class="btn" style="background:#ff5722; padding: 20px; font-size: 1.2em;">
                        🔥 挑戰祭壇<br><span style="font-size:0.8em">高難度挑戰</span>
                    </button>
                </div>
            `;
        }
    ...
    renderTrainingGrounds() {
        ...
        const html = `
            <div style="text-align:center; padding:20px;">
                ...
                <p style="font-size: 1.2em; margin-bottom: 20px;">消耗 EP 永久提升基礎能力</p>
                <p style="font-size: 1.2em; margin-bottom: 30px;">目前 EP: <span style="color:#69f0ae">${Player.explorationPoints}</span></p>
                <hr style="border-color:#444; margin:20px 0;">
                
                <div style="margin-bottom:30px; font-size: 1.2em;">
                    <p><strong>基礎攻擊力: +${Player.baseAtkBonus}</strong></p>
                    <button onclick="window.Game.upgradeBaseStats('atk')" class="btn" style="margin-top: 10px;" ${Player.explorationPoints < costAtk ? 'disabled' : ''}>
                        強化攻擊 (-${costAtk} EP)
                    </button>
                </div>
                
                <div style="margin-bottom:30px; font-size: 1.2em;">
                    <p><strong>基礎生命值: +${Player.baseHpBonus}</strong></p>
                    <button onclick="window.Game.upgradeBaseStats('hp')" class="btn" style="margin-top: 10px;" ${Player.explorationPoints < costHp ? 'disabled' : ''}>
                        強化生命 (-${costHp} EP)
                    </button>
                </div>
