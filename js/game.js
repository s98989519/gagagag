...
const Game = {
    ...
    startNewAdventure(startDepth = 1) {
        ...
        // 注意：不再強制清空 material，允許玩家從倉庫帶入素材
        Player.buff = null;
    ...
    donateItem(itemName) {
        ...
        // 優先檢查倉庫
        if (Player.warehouse[itemName] && Player.warehouse[itemName] > 0) {
        ...
        // 檢查背包
        const idx = Player.inventory.material.findIndex(i => i.name === itemName);
        if (idx !== -1) {
    ...
    handleExtraction() {
        // 計算獎勵
        let epReward = Math.floor(Player.depth / 10 * window.CONFIG.extractionBonusMultiplier);
